...
import { CheckCircle2, Edit2, ExternalLink, FileText, Image as ImageIcon, Plus, Printer, ShoppingBag, Trash2, Truck, Wallet, Info as InfoIcon } from "lucide-react";
  ...
  return (
    <div className="space-y-4 fade-in">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <Guide title="1. Buat Order" text="Catat PO supplier atau order marketplace. Simpan nomor order, link olshop, invoice, dan estimasi datang." />
        <Guide title="2. Terima Stok" text="Klik Terima Stok setelah barang sampai. Stok inventori/gudang baru bertambah pada tahap ini." />
        <Guide title="3. Bayar Bertahap" text="Bayar awal, cicilan, atau pelunasan dicatat terpisah agar utang supplier dan arus kas rapi." />
      </div>
      ...
      <Dialog open={showPO} onOpenChange={setShowPO}><DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>{editPO?'Edit PO':'Purchase Order Baru'}</DialogTitle></DialogHeader><PurchaseForm form={poForm} setForm={setPoForm} inventory={inventory} suppliers={suppliers} isPO/><DialogFooter><Button variant="outline" onClick={()=>setShowPO(false)}>Batal</Button><Button onClick={savePO}>Simpan</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={showOnline} onOpenChange={setShowOnline}><DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>{editOnline?'Edit Order Online':'Order Online Baru'}</DialogTitle></DialogHeader><PurchaseForm form={onlineForm} setForm={setOnlineForm} inventory={inventory}/><DialogFooter><Button variant="outline" onClick={()=>setShowOnline(false)}>Batal</Button><Button onClick={saveOnline}>Simpan</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={!!payTarget} onOpenChange={()=>setPayTarget(null)}><DialogContent className="max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>Catat Pembayaran</DialogTitle></DialogHeader>{payTarget&&<div className="text-sm text-gray-600">Sisa: <b>{formatRupiah((payTarget.doc.total||0)-(payTarget.doc.paid_amount||0))}</b></div>}<Field label="Nominal Bayar" type="number" value={payForm.amount} onChange={(v)=>setPayForm({...payForm,amount:v})}/><SelectField label="Metode" value={payForm.method} onChange={(v)=>setPayForm({...payForm,method:v})} items={PAYMENT_METHODS}/><ImageUpload value={payForm.payment_proof_url} onChange={(v)=>setPayForm({...payForm,payment_proof_url:v})} label="Bukti Bayar / Transfer" testid="purchase-payment-proof"/><Field label="Catatan" value={payForm.notes} onChange={(v)=>setPayForm({...payForm,notes:v})}/><DialogFooter><Button variant="outline" onClick={()=>setPayTarget(null)}>Batal</Button><Button onClick={savePayment}>Simpan Pembayaran</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={!!detail} onOpenChange={()=>{setDetail(null);setPayments([]);}}><DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>Detail Pembelian</DialogTitle></DialogHeader>{detail&&<PurchaseDetail kind={detail.kind} doc={detail.doc} payments={payments} onPrint={printPurchase}/>}<DialogFooter><Button variant="outline" onClick={()=>setDetail(null)}>Tutup</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}

function Guide({ title, text }) { return <div className="bg-white rounded-xl border border-gray-100 p-3"><div className="flex items-center gap-1.5 text-sm font-semibold text-gray-900"><InfoIcon className="w-4 h-4 text-[#1a6b3c]" />{title}</div><div className="text-xs text-gray-500 mt-1 leading-relaxed">{text}</div></div>; }
